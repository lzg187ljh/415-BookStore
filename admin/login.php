<?php
    ...
    if(isset($_POST['action'])){
        ...
        if(count($res) > 0){
            ...
            session_start();
            $_SESSION['username'] = $res[0]['manager_name'];
            $_SESSION['id'] = $res[0]['manager_id'];
...
<!-- login form -->
    ...
    <a class="brand" href="../BookStore/src/index.php"><i class="fas fa-book-open fa-7x mx-3"></i></a>
    </div>
    <div style="display:inline-block;padding-top:160px;background-image:linear-gradient(120deg,#6fc9b7,#093637);
    position:relative;height:100%;width:100%;display:flex;flex-direction:column;align-items:center;font-family: var(--Rubik);">
        <h3 style="color:white;font-weight:bold;font-size:3em">Book Shop Management</h3>
        <form action="" method="post" style="width:30%;">
        <div class="form-group" style="margin-top:5vmin;height:5vmin">
            
            <input type="text" class="form-control h-100" placeholder="Email" id="username" name="username">
        </div>
...
<!-- login form -->
    <script src="../BookStore/js/jquery-3.5.1.min.js"></script>   
    <script src="../BookStore/js/popper.min.js"></script>
