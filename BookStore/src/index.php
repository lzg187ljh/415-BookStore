...
<?php
  ...
  foreach($res as $k=>$row){
    $ISBN = $row['ISBN'];
    $sql = "select distinct * from cart a join cart_detail b on a.cart_id = b.cart_id where a.flag = 'Y' and b.ISBN = '$ISBN'";
    $resForAmount = select($connect,$sql);
    $amount = 0;
    if(count($resForAmount) > 0){
      foreach($resForAmount as $row){
  ...
  $news_key = array_column($res, 'amount');
  
  array_multisort($news_key, SORT_DESC, $res);
?>
      ...
      <section class="section-2">
          ...
          <div class="container text-center ">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="pray">
                <img src="../../file/<?php echo $res[0]['book_cover']?>" style="height:500px" alt="pray"/>
                </div>
              </div>
              <div class="col-md-6">
                <!-- panel -->
                <div class="panel text-left">
                  ...
                  <h1><?php echo $res[0]['book_author']?></h1>
                  <!-- padding top: 2 -->
                  <p class="pt-2"><?php echo $res[0]['book_info']?></p>
                  <p class="pt-2">sales volume <?php echo $res[0]['amount']?></p>
                  <a href="./bookDetail.php?id=<?php echo $res[0]['ISBN']?>"><button class="btn btn-light px-5 py-2">See more</button></a>
                </div>
              </div>
            </div> 
          </div> 
